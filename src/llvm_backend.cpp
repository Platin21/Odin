...
LLVMMetadataRef lb_debug_type_internal(lbModule *m, Type *type) {
	...
	case Type_BitFieldValue:
		{
			GB_PANIC("Type_BitFieldValue should be handled elsewhere");
			char i_name[16] = {};
			gb_snprintf(i_name, gb_size_of(i_name), "i%u", size);
			return LLVMDIBuilderCreateBasicType(m->debug_builder, i_name, gb_strlen(i_name), size, 0, LLVMDIFlagZero);
		}
	...
	case Type_BitField:
		{
			GB_ASSERT(type->BitField.fields.count == type->BitField.sizes.count);
			unsigned field_count = cast(unsigned)type->BitField.fields.count;
			LLVMMetadataRef *fields = gb_alloc_array(permanent_allocator(), LLVMMetadataRef, field_count);
			...
			for_array(i, type->BitField.sizes) {
				...
				LLVMMetadataRef field = LLVMDIBuilderCreateBitFieldMemberType(m->debug_builder, scope,
					name_text, name_len,
					file, field_line,
					cast(u64)size, cast(u64)offset, cast(u64)offset,
					LLVMDIFlagZero, i_type
				);
			...
			return LLVMDIBuilderCreateStructType(m->debug_builder, scope, "", 0, file, line,
				8*cast(u64)type_size_of(type), 8*cast(u32)type_align_of(type),
